...
st.set_page_config(page_title="Campanha de E-commerce — Avaliação (Recall)", layout="wide")
...
with st.expander("ℹ️ O que este app faz?", expanded=True):
    st.markdown(
        ...
        2. Exibe **Acurácia, Precisão e Recall** e os **valores da matriz de confusão (VP, FP, FN, VN)**.  
        3. **Não** mostra as fórmulas — os alunos devem **reproduzir as contas** a partir dos valores.  
...
# Helper OneHotEncoder compatível com versões diferentes do scikit-learn
def make_ohe():
    try:
...
if df_train is not None:
    ...
    # Métricas resumidas (sem fórmulas)
    ...
    # VALORES da matriz de confusão (binário)
    labels = np.unique(y_test)
    cm = confusion_matrix(y_test, y_pred, labels=labels)
    st.markdown("#### 🔢 Valores da Matriz de Confusão (classe positiva = 1)")
    if set(labels) == {0, 1}:
        vn, fp, fn, vp = int(cm[0,0]), int(cm[0,1]), int(cm[1,0]), int(cm[1,1])
        df_cm_vals = pd.DataFrame(
            cm,
            index=[f"Real {int(labels[0])}", f"Real {int(labels[1])}"],
            columns=[f"Prev {int(labels[0])}", f"Prev {int(labels[1])}"]
        )
        st.table(df_cm_vals)
        st.caption(f"VP = {vp} • FP = {fp} • FN = {fn} • VN = {vn}")
    else:
        st.info("Problema multiclasse detectado — esta seção é focada em binário {0,1}.")
...
st.markdown("---")
st.caption("Os alunos devem calcular Acurácia, Precisão e Recall a partir dos valores da matriz de confusão.")
