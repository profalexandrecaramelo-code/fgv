...
# Separar texto e valor
X_text = df["transacao"]
X_valor = df[["valor"]]
y = df["classe"]

# Pipeline VSM + SVM + scaler no valor
text_pipe = Pipeline([
...
if st.button("Classificar"):
    ...
    if resultado == "fraude":
        st.error(f"🚨 Esta transação foi classificada como **FRAUDE** com confiança: {prob_dict['fraude']:.2%}")
    else:
        st.success(f"✅ Esta transação foi classificada como **NORMAL** com confiança: {prob_dict['normal']:.2%}")
